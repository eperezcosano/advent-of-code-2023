...
function checkUpEdge() {
    return grid[0].reduce((max, _, x) => Math.max(max, startBeam([-1, x, 'down'])), 0)
}

function checkDownEdge() {
    return grid[0].reduce((max, _, x) => Math.max(max, startBeam([grid.length, x, 'up'])), 0)
}

function checkLeftEdge() {
    return grid.reduce((max, _, y) => Math.max(max, startBeam([y, -1, 'right'])), 0)
}

function checkRightEdge() {
    return grid.reduce((max, _, y) => Math.max(max, startBeam([y, grid[0].length, 'left'])), 0)
}

lineReader.on('line', (line) => grid.push(line.split('')))

lineReader.on('close', () => {
    const res = Math.max(checkUpEdge(), checkDownEdge(), checkLeftEdge(), checkRightEdge())
